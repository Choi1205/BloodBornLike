...
void ALadyMariaAIController::Tick(float DeltaTime)
{
	...
	if (!bIsStun && attackState == EAttackState::IDLE) {
		if (EnemyREF->phaseState == EPhaseState::PHASE3 && EnemyREF->GetPlayerDistance() > 1000.0f && stamina > 700.0f && !bIsForwardDodge) {
		...
		else if (EnemyREF->GetPlayerDistance() > 500.0f && EnemyREF->GetPlayerDistance() < 1000.0f && stamina > 700.0f && EnemyREF->phaseState != EPhaseState::PHASE1 && chargeTimer <= 0.0f) {
		...
		if (EnemyREF->GetPlayerDistance() >= 200.0f && EnemyREF->GetPlayerDistance() < 500.0f && bIsForwardDodge && stamina > 700.0f) {
			if (!EnemyREF->AnimInstance->IsAnyMontagePlaying()) {
				EnemyREF->ForwardDodge();
			}
		}
		//플레이어와 거리가 멀거나, 스테미너가 소모된 상황일경우
		if (EnemyREF->GetPlayerDistance() >= 200.0f || stamina < 1000.0f) {
			EnemyREF->WalkToPlayer();
		}

		//거리 200이하, 공격가능 스테미나, 플레이어의 속도가 100미만인 경우
		if (EnemyREF->GetPlayerDistance() < 200.0f && stamina > 700.0f && EnemyREF->GetPlayerSpeed() < 100.0f && chargeTimer <= 0.0f) {
			//패링 카운터로 찌르기가 나간다.
			attackState = EAttackState::QUICKTHRUST;
			chargeTimer = 5.0f;
		}

		if (EnemyREF->GetPlayerDistance() < 200.0f && stamina > 700.0f && attackState == EAttackState::IDLE) {
			if (RandomNextMoveTF(50)) {
